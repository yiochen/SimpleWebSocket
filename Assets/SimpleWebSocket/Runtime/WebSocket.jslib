...
var LibraryWebSocket = {
  ...
  $errorCode: {
    ...
    ALREADY_CONNECTING_OR_CONNECTED: -2,
    NOT_CONNECTED: -3,
    ALREADY_CLOSING: -4,
    ALREADY_CLOSED: -5,
    NOT_OPEN: -6,
  ...
  WebSocketAllocate: function (url) {
    console.log("Allocate new WebSocket");
  ...
  WebSocketFree: function (instanceId) {
    ...
    // Close if not closed
    if (instance.ws && instance.ws.readyState < readyState.CLOSING) {
      instance.ws.close();
    }
  ...
  WebSocketConnect: function (instanceId) {
    ...
    if (instance.ws !== null) {
      return errorCode.ALREADY_CONNECTING_OR_CONNECTED;
    }
  ...
  WebSocketClose: function (instanceId, code, reasonPtr) {
    ...
    if (instance.ws.readyState === readyState.CLOSING) {
      return errorCode.ALREADY_CLOSING;
    }

    if (instance.ws.readyState === readyState.CLOSED) {
      return errorCode.ALREADY_CLOSED;
    }
  ...
  WebSocketSend: function (instanceId, bufferPtr, length) {
    ...
    if (instance.ws.readyState !== readyState.OPEN) {
...
autoAddDeps(LibraryWebSocket, "$readyState");
autoAddDeps(LibraryWebSocket, "$errorCode");
